...
upd();
    ...
    # ---- hero atmosphere: particle shield (three.js) ----
    HERO_JS_TEMPLATE = """<script type="module">
(async function(){
'use strict';
const canvas=document.getElementById('hero-canvas'),hero=document.querySelector('.hero');
if(!canvas||!hero)return;
if(matchMedia('(prefers-reduced-motion: reduce)').matches)return;
let started=false;
new IntersectionObserver(es=>es.forEach(async e=>{
...
function init(THREE){
 ...
 // --- shield: particles sampled on a heraldic shield surface + orbiting ring
 // shield outline via 2d canvas sampling (gives a real shield silhouette, not a primitive)
 const off=document.createElement('canvas');off.width=200;off.height=240;
 const c=off.getContext('2d');
 c.beginPath();
 c.moveTo(100,8);            // top center
 c.lineTo(186,36);           // top right
 c.bezierCurveTo(188,130,168,196,100,232);   // right curve to point
 c.bezierCurveTo(32,196,12,130,14,36);       // left curve
 c.closePath();c.fillStyle='#fff';c.fill();
 const img=c.getImageData(0,0,200,240).data;
 const samples=[];
 while(samples.length<2600){
  const x=Math.random()*200,y=Math.random()*240;
  if(img[(y|0)*200*4+(x|0)*4+3]>128)samples.push([x/100-1,-(y/120-1)]);
 }
 const N=samples.length,total=N+900;
 const pos=new Float32Array(total*3),col=new Float32Array(total*3),seed=new Float32Array(total);
 const green=new THREE.Color('#3ddc97'),blue=new THREE.Color('#4fc3f7'),white=new THREE.Color('#9fd8ff');
 for(let i=0;i<N;i++){
  const [sx,sy]=samples[i];
  // give the flat silhouette gentle depth: bulge at center like a real shield
  const z=Math.cos(sx*1.35)*.85*(1-Math.abs(sy)*.25)+.4;
  pos[i*3]=sx*3.6;pos[i*3+1]=sy*2.5;pos[i*3+2]=z;
  const t=Math.random(),cc=t<.72?green:(t<.92?blue:white);
  col[i*3]=cc.r;col[i*3+1]=cc.g;col[i*3+2]=cc.b;
  seed[i]=Math.random()*Math.PI*2;
 }
 // orbital debris ring around the shield
 for(let i=N;i<total;i++){
  const k=(i-N)/900,a=k*Math.PI*2*7,r=4.6+Math.random()*.7,h=(Math.random()-.5)*.55;
  pos[i*3]=Math.cos(a)*r;pos[i*3+1]=h;pos[i*3+2]=Math.sin(a)*r;
  const t=Math.random(),cc=t<.6?blue:(t<.85?green:white);
  col[i*3]=cc.r;col[i*3+1]=cc.g;col[i*3+2]=cc.b;
  seed[i]=a;
 }
 ...
 const mat=new THREE.PointsMaterial({size:.09,vertexColors:true,transparent:true,opacity:.8,
  blending:THREE.AdditiveBlending,depthWrite:false,sizeAttenuation:true});
 const pts=new THREE.Points(geo,mat);
 const group=new THREE.Group();group.add(pts);scene.add(group);

 // faint wireframe shield echo behind the points
 const echoGeo=new THREE.BufferGeometry();
 echoGeo.setAttribute('position',geo.getAttribute('position'));
 const echo=new THREE.Points(echoGeo,new THREE.PointsMaterial({color:0x3ddc97,size:.02,transparent:true,opacity:.18}));
 group.add(echo);

 // --- interaction: pointer/tilt parallax + scroll fade (no UI to learn)
 let px=0,py=0,tpx=0,tpy=0;
 addEventListener('pointermove',e=>{tpx=(e.clientX/innerWidth-.5)*2;tpy=(e.clientY/innerHeight-.5)*2;},{passive:true});
 addEventListener('deviceorientation',e=>{
  if(e.gamma!=null){tpx=Math.max(-1,Math.min(1,e.gamma/30));tpy=Math.max(-1,Math.min(1,((e.beta||45)-45)/30));}
 },{passive:true});
 let scrollFade=1;
 addEventListener('scroll',()=>{
  const r=hero.getBoundingClientRect();
  scrollFade=Math.max(0,Math.min(1,(r.bottom+60)/(r.height*.8)));
 },{passive:true});
 ...
 // position: right of center on wide screens (clear of headline), centered on mobile
 function frame(){
 ...
 let visible=true;
 new IntersectionObserver(es=>es.forEach(e=>{visible=e.isIntersecting;}),{threshold:0}).observe(hero);
 const clock={t:0};
 function loop(now){
  requestAnimationFrame(loop);
  if(!visible)return;
  clock.t=now*.001;const s=clock.t;
  px+=(tpx-px)*.045;py+=(tpy-py)*.045;
  group.rotation.y=s*.22+px*.42;
  group.rotation.x=-py*.28;
  group.position.y=Math.sin(s*.6)*.22;
  // shimmer: shield particles breathe on their own axes
  const p=geo.attributes.position.array;
  for(let i=0;i<N;i+=4){p[i*3+2]+=Math.sin(s*1.4+seed[i])*.004;}
  geo.attributes.position.needsUpdate=true;
  // ring counter-rotates
  pts.geometry.drawRange?null:null;
  group.children[0].rotation.y=-s*.05;
  mat.opacity=.95*scrollFade;
  echo.material.opacity=.18*scrollFade;
  camera.lookAt(0,0,0);
...
<div class="hero" data-reveal><canvas id="hero-canvas" aria-hidden="true"></canvas><div class="wrap">
